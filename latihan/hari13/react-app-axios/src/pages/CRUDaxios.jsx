   ...
   const handleDelete = async (id) => {
    try {
        ...
        if (confirmDelete) {
            ...
            fetchDataMovie(); // refresh data setelah delete
        }
    } catch (err) {
        console.error(err);
    ...
    return(
      ...
      <div className="card-body">
        <form onSubmit={handleSubmit}>
            ...
            <input type="submit" value="Submit" className='btn bg-green-400 hover:bg-green-800 mt-4' />
        </form>
        </div>
        </div>
                ...
                {dataMovie.map((item, index)=>{
                    return(
                        <tr key={index}>
                            ...
                            <td><input type="submit" value="Ubah" className='btn bg-blue-600 hover:bg-blue-700 text-black' onClick={()=>handleEdit(item.id)}/></td>
                            <td><input type="submit" value="Hapus" className='btn bg-red-600 hover:bg-red-700 text-white'onClick={()=>handleDelete(item.id)}/></td>
                        </tr>
                )
                })}
            </tbody>
